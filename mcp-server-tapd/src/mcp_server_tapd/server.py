...
def get_user_participant_projects(nick: str = '') -> str:
    ...
    category的值如果是organization，代表是公司，如果用户想要获取参与的项目，要过滤掉 category 为 organization 的数据
    """
...
@mcp.tool()
def get_entity_attachments(workspace_id: int,options: dict = None) -> str:
    """获取需求/缺陷/任务的附件信息，包括下载链接，下载链接默认有效时间300s

    Args:
        - workspace_id: 项目ID（必填）
        - options: 参数字典，支持以下字段：
            - entry_id: 业务对象ID（必填）
            - type: 业务对象类型，需求 story 或缺陷 bug（必填）


    Returns:  <str>  #  包含附件下载信息的json格式数据，该数据包含如下字段：
        id	附件ID
        type	业务对象类型
        entry_id	业务对象的id
        filename	文件名
    ...
    ret = client.get_attachments(params)
...
@mcp.tool()
def create_tcases_batch(workspace_id: int, options: dict = None) -> str:
    """批量新建测试用例，返回新建测试用例的数据
    Args:
        workspace_id(int): 项目ID（必填）
        options: 可选参数字典，支持以下字段：
            - tcases(list): 测试用例列表，每个元素为测试用例字典，支持字段：
                - workspace_id(int): 项目ID（可选，默认使用外部传入的 ID）
                - name(str): 用例名称（必填）
                - category_id(int): 用例目录 ID
                - status(str): 用例状态，取值 updating, abandon, normal
    ...
    data = []
    if options and 'tcases' in options:
        data = options['tcases']
    
    if not data:
        return json.dumps({"status": 0, "info": "tcases list is empty"}, indent=2, ensure_ascii=False)
    
    if len(data) > 200:
        return json.dumps({"status": 0, "info": "每次新增最大为两百"}, indent=2, ensure_ascii=False)

    for tcase in data:
        if 'workspace_id' not in tcase:
            tcase['workspace_id'] = workspace_id
            
        if 'precondition' in tcase:
            html = markdown.markdown(tcase["precondition"], extensions=['extra', 'codehilite', 'toc'])
            if '<p>' in html or '<h' in html or '<ul>' in html or '<ol>' in html:
                tcase["precondition"] = html
        if 'steps' in tcase:
            html = markdown.markdown(tcase["steps"], extensions=['extra', 'codehilite', 'toc'])
            if '<p>' in html or '<h' in html or '<ul>' in html or '<ol>' in html:
                tcase["steps"] = html
        if 'expectation' in tcase:
            html = markdown.markdown(tcase["expectation"], extensions=['extra', 'codehilite', 'toc'])
            if '<p>' in html or '<h' in html or '<ul>' in html or '<ol>' in html:
                tcase["expectation"] = html
                
    created_tcases_batch = client.create_tcases_batch_save(data)
    return json.dumps(created_tcases_batch, indent=2, ensure_ascii=False)
