...
class TAPDClient:
            ...
            for item in result['data']:
                if 'Attachment' in item:
                    ...
                    if attachment_id:
                        try:
                            # 调用获取下载链接接口
                            download_result = self.get_attachment_download_url({
                                'id': attachment_id,
                                'workspace_id': workspace_id
                            })
                            
                            # 将下载链接添加到附件信息中
                            if download_result.get('status') == 1 and download_result.get('data'):
                                # 下载链接可能在 data.Attachment.download_url 或 data.download_url 中
                                data = download_result['data']
                                download_url = None
                                
                                if isinstance(data, dict):
                                    if 'Attachment' in data and isinstance(data['Attachment'], dict):
                                        download_url = data['Attachment'].get('download_url')
                                    else:
                                        download_url = data.get('download_url')
                                
                                if download_url:
                                    # 直接修改 result 中的数据
                                    item['Attachment']['download_url'] = download_url
                                else:
                                    # 调试信息：记录下载结果
                                    item['Attachment']['download_url'] = None
                                    item['Attachment']['_debug_download_result'] = download_result
                            else:
                                # API调用失败
                                item['Attachment']['download_url'] = None
                                item['Attachment']['_debug_error'] = f"API返回状态: {download_result.get('status')}, info: {download_result.get('info')}"
                                
                        except Exception as e:
                            # 如果获取下载链接失败，继续处理其他附件
                            item['Attachment']['download_url'] = None
                            item['Attachment']['_debug_exception'] = str(e)
        
        return result
    
    def get_attachment_download_url(self, params: Optional[Dict] = None) -> dict:
        """
        获取附件的下载链接。每次只能请求一个附件的下载链接，下载链接默认有效时间300s
        参数:
            - id: 附件ID(必填)
        ...
        return self._make_request("GET", "attachments/down", params=default_params)
    ...
    def create_tcases_batch_save(self, data: List[Dict[str, Any]]) -> Dict:
        """
        批量新建测试用例
        """
        if getattr(self, "nick", None):
