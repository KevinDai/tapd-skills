...
在配置好环境变量（TAPD_ACCESS_TOKEN 或 TAPD_API_USER/TAPD_API_PASSWORD；TAPD_API_BASE_URL 可选，默认云环境）后，可直接在项目根或 `skills/tapd/scripts/` 下运行以下命令，**无需安装 MCP 或第三方依赖**（仅需 Python 3 与标准库）。输出为 JSON 到 stdout，便于解析。
